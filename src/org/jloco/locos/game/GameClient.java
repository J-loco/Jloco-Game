...
import org.jloco.locos.common.CryptManager;
import org.jloco.locos.entity.exchange.NpcExchange;
import org.jloco.locos.entity.map.*;
import org.jloco.locos.game.action.type.BigStoreActionData;
import org.jloco.locos.game.action.type.DocumentActionData;
import org.jloco.locos.game.action.type.NpcDialogActionData;
import org.jloco.locos.game.action.type.ScenarioActionData;
import org.jloco.locos.hdv.BigStore;
import org.jloco.locos.hdv.BigStoreListingLotSize;
import org.jloco.locos.script.DataScriptVM;
import org.jloco.locos.util.Pair;
import org.apache.mina.core.session.IoSession;
import org.jloco.locos.area.Area;
import org.jloco.locos.area.map.GameCase;
import org.jloco.locos.area.map.GameMap;
import org.jloco.locos.auction.AuctionManager;
import org.jloco.locos.client.Account;
import org.jloco.locos.client.Player;
import org.jloco.locos.client.other.Party;
import org.jloco.locos.command.CommandAdmin;
import org.jloco.locos.command.CommandPlayer;
import org.jloco.locos.command.administration.AdminUser;
import org.jloco.locos.common.Formulas;
import org.jloco.locos.common.PathFinding;
import org.jloco.locos.common.SocketManager;
import org.jloco.locos.database.DatabaseManager;
import org.jloco.locos.database.data.game.*;
import org.jloco.locos.database.data.login.*;
import org.jloco.locos.entity.Collector;
import org.jloco.locos.entity.Prism;
import org.jloco.locos.entity.exchange.CraftSecure;
import org.jloco.locos.entity.exchange.Exchange;
import org.jloco.locos.entity.exchange.PlayerExchange;
import org.jloco.locos.entity.mount.Mount;
import org.jloco.locos.entity.npc.Npc;
import org.jloco.locos.entity.npc.NpcTemplate;
import org.jloco.locos.entity.pet.Pet;
import org.jloco.locos.entity.pet.PetEntry;
import org.jloco.locos.event.EventManager;
import org.jloco.locos.event.type.Event;
import org.jloco.locos.fight.Fight;
import org.jloco.locos.fight.Fighter;
import org.jloco.locos.fight.spells.Spell;
import org.jloco.locos.game.action.ExchangeAction;
import org.jloco.locos.game.action.GameAction;
import org.jloco.locos.game.world.World;
import org.jloco.locos.game.world.World.Couple;
import org.jloco.locos.guild.Guild;
import org.jloco.locos.guild.GuildMember;
import org.jloco.locos.hdv.BigStoreListing;
import org.jloco.locos.job.Job;
import org.jloco.locos.job.JobAction;
import org.jloco.locos.job.JobConstant;
import org.jloco.locos.job.JobStat;
import org.jloco.locos.job.maging.BreakingObject;
import org.jloco.locos.job.maging.Rune;
import org.jloco.locos.kernel.Config;
import org.jloco.locos.kernel.Constant;
import org.jloco.locos.kernel.Logging;
import org.jloco.locos.kernel.Main;
import org.jloco.locos.lang.LangEnum;
import org.jloco.locos.object.GameObject;
import org.jloco.locos.object.ObjectTemplate;
import org.jloco.locos.object.entity.Fragment;
import org.jloco.locos.object.entity.SoulStone;
import org.jloco.locos.util.TimerWaiter;
import org.jloco.locos.util.generator.NameGenerator;
...
public class GameClient {
    ...
    private void authorisedCommand(String packet) {
        ...
        if (this.getPlayer() == null || this.player.getGroup() == null) {
            this.player.sendMessage("Vous n'avez pas les droits.");
            return;
        }
    ...
    private synchronized void movementItemOrKamas(String packet) {
        ...
        switch(this.player.getExchangeAction().getType()) {
            ...
            case ExchangeAction.CRAFTING:
                if (!(this.player.getExchangeAction().getValue() instanceof JobAction)) break;
                JobAction craftJobAction = (JobAction) this.player.getExchangeAction().getValue();

                if (packet.charAt(2) == 'O' && craftJobAction.getJobCraft() == null) {
                    String craftPacket = packet.replace("-", ";-").replace("+", ";+").substring(4);
                    for (String part : craftPacket.split(";")) {
                        try {
                            if (part.isEmpty()) continue;
                            char c = part.charAt(0);
                            String[] infos = part.substring(1).split("\\|");
                            int id = Integer.parseInt(infos[0]), quantity = 1;
                            try { quantity = Integer.parseInt(infos[1]); } catch (Exception ignored) {}
                            if (quantity <= 0) return;
                            if (c == '+') {
                                if (!this.player.hasItemGuid(id)) return;
                                GameObject obj = this.player.getItems().get(id);
                                if (obj == null || obj.getObvijevanLook() != 0) { player.send("BN"); return; }
                                if (obj.getQuantity() < quantity) quantity = obj.getQuantity();
                                craftJobAction.addIngredient(this.player, id, quantity);
                            } else if (c == '-') {
                                craftJobAction.addIngredient(this.player, id, -quantity);
                            }
                        } catch (Exception e) {
                    ...
                    if (craftJobAction.getJobCraft() == null)
                        craftJobAction.setJobCraft(craftJobAction.oldJobCraft);
                    if (craftJobAction.getJobCraft() != null)
                        craftJobAction.getJobCraft().setAction(Integer.parseInt(packet.substring(3)));
                } else if (packet.charAt(2) == 'r') {
                    if (craftJobAction.getJobCraft() != null)
                        craftJobAction.broke = true;
                }
                break;
...
//                    }
            ...
            case ExchangeAction.TRADING_WITH_COLLECTOR:
                ...
                for (Player loc : World.world.getGuild(collector.getGuildId()).getPlayers()) {
                    if (loc != null && loc.isOnline()) {
                        SocketManager.GAME_SEND_gITM_PACKET(loc, org.jloco.locos.entity.Collector.parseToGuild(loc.getGuild().getId()));
                        String str = "";
    ...
    private void removeTaxCollector(String packet) {
        ...
        for (Player z : this.player.getGuild().getPlayers()) {
            if (z.isOnline()) {
                SocketManager.GAME_SEND_gITM_PACKET(z, org.jloco.locos.entity.Collector.parseToGuild(z.getGuild().getId()));
                String str = "";
    ...
    private void placeTaxCollector() {
        ...
        for (Player player : guild.getPlayers()) {
            if (player != null && player.isOnline()) {
                SocketManager.GAME_SEND_gITM_PACKET(player, org.jloco.locos.entity.Collector.parseToGuild(player.getGuild().getId()));
                String str = "";
